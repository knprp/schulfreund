...
from PyQt6.QtCore import QDate, Qt, QSize  # Qt hier hinzugefügt
...
class SettingsTab(QWidget):
    ...
    def refresh_history_combo(self):
        """Aktualisiert die Combobox mit den gespeicherten Halbjahren"""
        self.semester_combo.clear()
        try:
            history = self.parent.db.get_semester_history()
            for semester in history:
                display_text = semester['name'] if semester['name'] else f"{semester['start_date']} - {semester['end_date']}"
                self.semester_combo.addItem(display_text, semester)
        except Exception as e:
            QMessageBox.critical(self, "Fehler", f"Fehler beim Laden der Historie: {str(e)}")

    def add_to_history(self):
        """Öffnet Dialog zum Hinzufügen eines Halbjahres zur Historie"""
        dialog = SemesterHistoryDialog(self)
        if dialog.exec():
            try:
                start_date = dialog.start_date.date().toString("yyyy-MM-dd")
                end_date = dialog.end_date.date().toString("yyyy-MM-dd")
                name = dialog.name.text().strip() or None
                notes = dialog.notes.toPlainText().strip() or None
                
                self.parent.db.save_semester_to_history(start_date, end_date, name, notes)
                self.refresh_history_combo()
                self.parent.statusBar().showMessage("Halbjahr wurde zur Historie hinzugefügt", 3000)
            except Exception as e:
                QMessageBox.critical(self, "Fehler", f"Fehler beim Speichern: {str(e)}")

    def load_from_history(self):
        """Lädt das ausgewählte Halbjahr aus der Historie"""
        current_data = self.semester_combo.currentData()
        if current_data:
            try:
                self.start_date.setDate(QDate.fromString(current_data['start_date'], "yyyy-MM-dd"))
                self.end_date.setDate(QDate.fromString(current_data['end_date'], "yyyy-MM-dd"))
            except Exception as e:
